...
//! loads DEM named 'demname' from surfit datafile (see \ref datafile for details)
GLOBE_EXPORT
d_dem * _dem_load(const char * filename, const char * demname);

//! loads DEM from DTM files (hdr + bin)
GLOBE_EXPORT
d_dem * _dem_load_dtm(const char * hdr_filename, const char * bin_filename = 0);

//! loads DEM from HGT files (Shuttle Radar Topographic Mission files)
GLOBE_EXPORT
...
//! loads DEM from GTOPO30 files (The Global Land One-km Base Elevation (GLOBE) Project)
GLOBE_EXPORT
...
//! loads DE< from SURFER grd file (ASCII format)
GLOBE_EXPORT
...
//! converts DEM to \ref d_surf
GLOBE_EXPORT
d_surf * _dem_to_surf(const d_dem * srf);

//! converts DEM to \ref d_mask
GLOBE_EXPORT
d_mask * _dem_to_mask(const d_dem * srf, short true_from, short true_to);

//! prints some info about DEM
GLOBE_EXPORT
void _dem_info(const d_dem * srf);

//! calculates residuals between values set at points in \ref d_points "points" and DEM "srf". Saves result to text file.
GLOBE_EXPORT
bool _dem_resid(const d_dem * srf, const d_points * tsk, const char * filename);

//! recalculates DEM "srf" to new \ref d_grid "grd" using bilinear interpolation algorithm
GLOBE_EXPORT
