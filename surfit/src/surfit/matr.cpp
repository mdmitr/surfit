...
void matr::mult(const REAL * b_begin, REAL * r_begin)
{
	int J;
	for (J = 0; J < rows(); J++) {
		*(r_begin+J) = mult_line(J, b_begin, b_begin+cols());
	}
	call_after_mult();
};

void matr::mult_transposed(const vec * b, vec * r) {
	int J;
	for (J = 0; J < cols(); J++) {
...
REAL matr_sum::element_at_transposed(int i, int j, int * next_j) const {

	REAL res = 0;
	int next_j1 = INT_MAX;
	int next_j2 = INT_MAX;
	if (T1 && w1)
		res += w1 * T1->element_at_transposed(i,j,&next_j1);
	if (cT1 && w1)
		res += w1 * cT1->element_at_transposed(i,j,&next_j1);
	if (T2 && w2)
		res += w2 * T2->element_at_transposed(i,j,&next_j2);
	if (cT2 && w2)
		res += w2 * cT2->element_at_transposed(i,j,&next_j2);
...
REAL matr_sum::at_transposed(int i, int j, int * next_j) const {

	REAL res = 0;
	int next_j1 = INT_MAX;
	int next_j2 = INT_MAX;
	if (T1 && w1)
		res += w1 * T1->at_transposed(i,j,&next_j1);
	if (cT1 && w1)
		res += w1 * cT1->at_transposed(i,j,&next_j1);
	if (T2 && w2)
		res += w2 * T2->at_transposed(i,j,&next_j2);
	if (cT2 && w2)
		res += w2 * cT2->at_transposed(i,j,&next_j2);
...
REAL matr_sum::mult_transposed_line(int J, const REAL * b_begin, const REAL * b_end) {
	REAL res = 0;
	if (T1 && w1)
		res += w1*T1->mult_transposed_line(J, b_begin, b_end);
	if (cT1 && w1)
		res += w1*cT1->mult_transposed_line(J, b_begin, b_end);
	if (T2 && w2)
		res += w2*T2->mult_transposed_line(J, b_begin, b_end);
	if (cT2 && w2)
		res += w2*cT2->mult_transposed_line(J, b_begin, b_end);
...
REAL matr_sums::element_at_transposed(int i, int j, int * next_j) const {
	...
	for (q = 0; q < (int)matrices->size(); q++) {
		matr * T = (*matrices)[q];
		REAL w = (*weights)(q);
		res += w * T->element_at_transposed(i,j,&next_j2);
		next_j1 = MIN(next_j1, next_j2);
	}
...
REAL matr_sums::at_transposed(int i, int j, int * next_j) const {
	...
	for (q = 0; q < (int)matrices->size(); q++) {
		...
		if (T == NULL) 
			continue;
		REAL w = (*weights)(q);
		res += w * T->at_transposed(i,j,&next_j2);
		next_j1 = MIN(next_j1, next_j2);
	}
...
REAL matr_sums::mult_transposed_line(int J, const REAL * b_begin, const REAL * b_end) {
	REAL res = 0;
	int q;
	for (q = 0; q < (int)matrices->size(); q++) {
		matr * T = (*matrices)[q];
		REAL w = (*weights)(q);
		res += w * T->mult_transposed_line(J, b_begin, b_end);
	}
	return res;
};
...
REAL matr_mask::element_at_transposed(int i, int j, int * next_j) const {
	...
	return matrix->element_at_transposed(i,j,next_j);
...
REAL matr_mask::at_transposed(int i, int j, int * next_j) const {
	...
	return matrix->at_transposed(i,j,next_j);
...
REAL matr_mask::mult_transposed_line(int J, const REAL * b_begin, const REAL * b_end) {
	
	if (mask)
	if (mask->get(J) == false) 
		return REAL(0);
	
	return matrix->mult_transposed_line(J, b_begin, b_end);
