					...
					if ( (z[k] >= dmin) && (z[k] <= dmax) ) {
						for ( m=4; m >= 0; m-- ) {
							if (m > 0) {
								...
								yh[m] = y[j+jm[m-1]];
								 ...
								 h[0] = REAL(0.25)*(h[1]+h[2]+h[3]+h[4]);
								xh[0] = REAL(0.50)*(x[i]+x[i+1]);
								yh[0] = REAL(0.50)*(y[j]+y[j+1]);
							}
							if (h[m]>0.0) {
								sh[m] = 1;
							} else if (h[m]<0.0) {
						...
						for ( m = 1; m <= 4; m++ ) {
							...
							if (case_value!=0) {
								...
								//printf("%f %f %f %f %f\n",x1,y1,x2,y2,z[k]);
								if ( 
									(show_y_from < x1) && (x1 <= show_y_to) &&
									(show_y_from < x2) && (x2 <= show_y_to) &&
									(show_x_from < y1) && (y1 <= show_x_to) &&
									(show_x_from < y2) && (y2 <= show_x_to)
									) 
								{
									if (z[k] != undef_value) {			
										glVertex3f(float(y1),float(x1),float(z[k]));
										glVertex3f(float(y2),float(x2),float(z[k]));
									}
								}
