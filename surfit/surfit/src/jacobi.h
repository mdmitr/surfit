...
#ifndef __surfit_solver_jacobi__
#define __surfit_solver_jacobi__
...
vec * jacobi(M * A, V * b, int max_it, REAL tol, vec *& X, REAL omega = REAL(1.6), REAL undef_value = FLT_MAX) {
	...
	writelog(SURFIT_MESSAGE,"jacobi: (%d)\terror : %12.6G iter : %d",x->size(), error, iter);
