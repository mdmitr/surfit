...
grid * method_grid;
grid * method_sub_grid;
grid * method_prev_grid;
vec * method_X;
...
grid * create_calc_grd(int basis_cntX, int basis_cntY) {
	...
	grid * grd = new grid(grd_startX+grd_stepX/REAL(2), 
...
grid * create_last_grd() {
	...
	grid * last_grd = new grid(grd_startX+grd_stepX/REAL(2), 
		                   grd_endX-grd_stepX/REAL(2), 
				   grd_stepX, 
				   grd_startY+grd_stepY/REAL(2), 
				   grd_endY-grd_stepY/REAL(2), 
...
void grid_user::prepare()
{
	...
	method_sub_grid = new grid((startx+endx)/REAL(2), (startx+endx)/REAL(2), (endx-startx),
				   (starty+endy)/REAL(2), (starty+endy)/REAL(2), (endy-starty));
};

void grid_user::begin() {
	...
	writelog(SURFIT_MESSAGE,"*************");
	if (map_name)
		writelog(SURFIT_MESSAGE,"* Phase %d, space dimension = %d x %d (%s)",
		         method_phase_counter, method_basis_cntX, method_basis_cntY, map_name);
	else
		writelog(SURFIT_MESSAGE,"* Phase %d, space dimension = %d x %d",
	...
	if (!method_X) 
		method_X = new vec(matrix_size);

	delete method_mask_solved;
	method_mask_solved = new bitvec(matrix_size);
	method_mask_solved->init_false();
	delete method_mask_undefined;
	method_mask_undefined = new bitvec(matrix_size);
...
void grid_user::stage1(int priority) {};
void grid_user::stage2(int priority) {};
void grid_user::stage3(int priority) {};
void grid_user::stage4(int priority) {};

void grid_user::stage5(int priority) {};

void grid_user::finish() {

	delete method_sub_grid;
	method_sub_grid = new grid(method_grid);

	delete method_prev_grid;
	method_prev_grid = new grid(method_grid);
...
void grid_user::release() {
	...
	func * res_func = new func(method_X, new grid(method_grid), map_name);
	delete method_grid;
	method_grid = NULL;
	method_X = NULL;
