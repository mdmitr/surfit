...
grid_line::grid_line(int * ifirst_begin, int * ifirst_end,
                     int * isecond_begin, int * isecond_end) {

	first_begin = ifirst_begin;
	first_end = ifirst_end;
	second_begin = isecond_begin;
	second_end = isecond_end;
...
void grid_line::sort() {
	...
	int * ptr;
	int ** pptr;
	for (ptr = first_begin, pptr = sort_by_first_begin; ptr != first_end; ptr++, pptr++)
		*pptr = ptr;
	...
	for (ptr = second_begin, pptr = sort_by_second_begin; ptr != second_end; ptr++, pptr++)
		*pptr = ptr;

	std::sort(sort_by_second_begin, sort_by_second_end, ptr_less);
};

void grid_line::add(int *& ifirst_begin, int *& ifirst_end,
                    int *& isecond_begin, int *& isecond_end) 
{
	if (ifirst_end - ifirst_begin != isecond_end - isecond_begin)
		return;

	int add_size = ifirst_end - ifirst_begin;
	int old_size = size();

	first_begin  = (int *) realloc(first_begin, (old_size+add_size)*sizeof(int));
	memcpy(first_begin + old_size, ifirst_begin, add_size*sizeof(int));
	free(ifirst_begin);
	ifirst_begin = NULL;
	ifirst_end = NULL;
	first_end = first_begin + add_size + old_size;

	second_begin = (int *) realloc(second_begin, (old_size+add_size)*sizeof(int));
	memcpy(second_begin + old_size, isecond_begin, add_size*sizeof(int));
	free(isecond_begin);
	isecond_begin = NULL;
	isecond_end = NULL;
	second_end = second_begin + add_size + old_size;
...
void grid_line::add(grid_line * line) {
	if (!line)
		return;
	add(line->first_begin, line->first_end,
		line->second_begin, line->second_end);
};
...
first_find_J:
	...
	if (first_y || second_y || first_x || second_x ) {
		...
		if (ptr && (ptr != fault->sort_by_first_end)) {
			if (**ptr == J) {
				pos = *ptr - fault->first_begin;
				J2 = *(fault->second_begin + pos);
...
second_find_J:
	...
	if (first_x || second_x || first_y || second_y) {
		...
		if (ptr && (ptr != fault->sort_by_second_end)) {
			if (**ptr == J) {
				pos = *ptr - fault->second_begin;
				J2 = *(fault->first_begin + pos);
				diff = abs(pos2find - J2);
...
void flood_fill(grid * grd,
	...
	intvec * flood_points = new intvec(0,0,0,MAX(grid_size/4,1));
		    ...
		    bitvec * mask_undefined)
{
	...
	if (line)
	{
		int fl_size = line->first_end - line->first_begin;
		int i;
		int J1, J2;
		for (i = 0; i < fl_size; i++) {
			J1 = *(line->first_begin + i);
			J2 = *(line->second_begin + i);
...
*/
	...
	if (!exists_undef)
		...
		bitvec * local_mask_undefined = new bitvec(nn*mm);
